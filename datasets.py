...
def load_dataset(root_dir, redux, params, shuffled=False, single=False,clean_targets=False):
    ...
    if params.noise_type == 'mc':
        dataset = MonteCarloDataset(root_dir, redux, params.patch_size,
            clean_targets=params.clean_targets)
    else:
        dataset = NoisyDataset(root_dir, redux, params.patch_size,params,
...
class AbstractDataset(Dataset):
    ...
    def __init__(self, root_dir, redux=0, patch_size=128, clean_targets=False):
    ...
    def _random_crop(self, img_list):
        ...
        w, h = img_list[0].size
        assert w >= self.patch_size and h >= self.patch_size, \
            f'Error: Crop size: {self.patch_size}, Image size: ({w}, {h})'
        cropped_imgs = []
        ...
        for img in img_list:
            ...
            if min(w, h) < self.patch_size:
                img = tvF.resize(img, (self.patch_size, self.patch_size))

            # Random crop
            cropped_imgs.append(tvF.crop(img, i, j, self.patch_size, self.patch_size))

        return cropped_imgs

    def get_patch(self, full_input_img, patch_n, patch_size):
        patch_input_imgs = []
...
class NoisyDataset(AbstractDataset):
    ...
    def __init__(self, root_dir, redux, patch_size, params,clean_targets=False,
        noise_dist=('gaussian', 50.), seed=None):
        """Initializes noisy image dataset."""

        super(NoisyDataset, self).__init__(root_dir, redux, patch_size, clean_targets)

        #self.imgs = os.listdir(root_dir)
        self.imgs = sorted(glob(os.path.join(root_dir, '*_input.npy')))
        self.clean_targets=clean_targets
        self.p = params
        if clean_targets:
            self.target = sorted(glob(os.path.join(root_dir, '*_target.npy')))
#        if redux:
#            self.imgs = self.imgs[:redux]
        ...
        self.noise_param_source=self.noise_param #+5
        self.seed = seed
        if self.seed:
    ...
    def _add_Hybrid_noise(self, img,std=10):
        """Adds Hybrid noise to image."""
        
        _,w, h = img.shape
#        c = len(img.getbands())

        if self.noise_type == 'hybrid':

            vals = len(np.unique(img))
            vals = 2 ** np.ceil(np.log2(vals))
            noise_img = np.random.poisson(img * vals) / float(vals)

            #std = self.noise_param
            noise_guss = np.random.normal(0, std, (h, w))
            noise_img = np.array(noise_img) + noise_guss
            noise_img = np.clip(noise_img, 0, 255).astype(np.uint8)
#            image = np.expand_dims(noise_img, axis=2)  # 单通道
#            image = np.concatenate((image, image, image), axis=-1)  # 单通道
            return [patch for patch in noise_img] #Image.fromarray(noise_img)
    ...
    def __getitem__(self, index):
        ...
        img= np.load(img_path)*255

        # Random square crop
        if self.patch_size != 0 and self.clean_targets==False:
            #img = img.astype(np.uint8)
            #img = Image.fromarray(img)
            img = self.get_patch(img,self.p.patch_n,self.patch_size) #self._random_crop([img])[0]
            #img = np.array(img)
            
        # Corrupt source image
        if self.clean_targets:
            source = tvF.to_tensor(img).float()/255
        else:
            #tmp = self._corrupt(img)
            source = torch.from_numpy(img).float().unsqueeze(1)/255

        # Corrupt target image, but not when clean targets are requested
        if self.clean_targets:
            target_img=np.load(self.target[index])*255
            target = tvF.to_tensor(target_img).float()/255
        else:
            target = torch.from_numpy(img).float().unsqueeze(1)/255
...
class MonteCarloDataset(AbstractDataset):
    ...
    def __init__(self, root_dir, redux, patch_size,
        hdr_buffers=False, hdr_targets=True, clean_targets=False):
        """Initializes Monte Carlo image dataset."""

        super(MonteCarloDataset, self).__init__(root_dir, redux, patch_size, clean_targets)
