...
class ct_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.patch_size:
            input_patches, _ = get_patch(input_img,
                                         target_img,
                                         self.patch_n,
                                         self.patch_size)
            input_patches_n=tvF.to_tensor(np.array(add_Hybrid_noise(input_patches*255)))/255
            #target_patches_n=tvF.to_tensor(np.array(add_Hybrid_noise(input_patches*255)))/255
            return (input_patches_n, input_patches)
        else:
            return (input_img, target_img)

def add_Hybrid_noise(img):
...
#            image = np.concatenate((image, image, image), axis=-1)  # 单通道
        return [Image.fromarray(patch) for patch in noise_img]
