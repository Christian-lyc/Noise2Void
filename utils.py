...
def progress_bar(batch_idx, num_batches, report_interval, train_loss):
    ...
    print('\rBatch {:>{dec}d} [{}{}] Train loss: {:>1.5f}'.format(batch_idx + 1, '=' * fill + '>', ' ' * (bar_size - fill), train_loss, dec=str(dec)), end='',flush=True)
...
def show_on_epoch_end(epoch_time, valid_time, valid_loss, valid_psnr,valid_ssim):
    """Formats validation error stats."""

    clear_line()
    print('Train time: {} | Valid time: {} | Valid loss: {:>1.5f} | Avg PSNR: {:.2f} dB | Avg SSIM: {:.2f}'.format(epoch_time, valid_time, valid_loss, valid_psnr,valid_ssim),flush=True)
...
def show_on_report(batch_idx, num_batches, loss, elapsed):
    ...
    clear_line()
    dec = int(np.ceil(np.log10(num_batches)))
    print('Batch {:>{dec}d} / {:d} | Avg loss: {:>1.5f} | Avg train time / batch: {:d} ms'.format(batch_idx + 1, num_batches, loss, int(elapsed), dec=dec),flush=True)
